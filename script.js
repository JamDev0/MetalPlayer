...
function MostrarC(v)
{
    ...
    console.log(v.target.attributes.id.value);
...
function PlayPause(a)
{
    if(c == 0)
    {
        if(window.innerHeight > 800 && a.toElement.attributes.id.value == 'Video')
        {  
            c = 1;
            PP.style.width = '40px';
            Video.play();
...
function att()
{
    ...
    TempoA.innerText = Math.floor(Video.currentTime) + 's - ' + Video.duration + 's';
    //Setar css
